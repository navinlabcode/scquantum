  ...
  annotations, chrom.colname, bin.start.colname, bin.end.colname)
{
  ...
  right.annotations <- if (is.null(bin.end.colname)) {data.frame()[1:nrow(annotations),]} else
  {
      ...
      while (any(colnames(out) %in% colnames(left.annotations)))
      {
        colnames(out)[colnames(out) %in% colnames(left.annotations)] <-
          sapply(colnames(out)[colnames(out) %in% colnames(left.annotations)], paste, "end", sep=".")
      }
      out
  }
  ...
  return(annotated.segmented.counts[,c(
    "penalty", colnames(left.annotations), colnames(right.annotations), "mean", "se", "length"
  )])
}
